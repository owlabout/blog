...
# Organizing Node.js modules in your Vue.js Project
...
I was wrong. In fact, npm comes with some cool extra fun little features, with which help you manage your modules in a way, that barely influences your standard development workflow. In the following I'm going to go through the steps to move some Vue-components of your Vue.js project to a node-module and continue on as before. 
...
import { ModuleA } from "module-a";
```
## Use peer-dependencies libraries and dependencies for plug-ins
...
For this, npm lets us use "peer depencencies". Peer depencencies will let us specify what the core project needs in order to use the module. By adding a peer dependency you can actually tell npm: "If I'm being used by an another project, please make sure that the project is using Vue at a minimum verion of 2.x". If you then install the module in a project that does not fulfill those requirements it will simply give you a warning, that you are missing a peer-dependency. 

In my case, module A will never be used alone anyway. It is meant purely as a plugin for my core project, so there is really no reason for it to care, which version of Vue is being used, it acutally only cares that the core-project is being used. So the only dependecy I need to put in it, is one linking it to my core project. 
Module A is also currently included IN the core-project directory, meaning that it has full access to any and all modules in the node_modules directory of the core-project. If I were to move the modules somehwere else, then I would have to add vue as an extra dependency or a peer-dependency.
...
While this would work just fine, it is very inconvinient for the development of module A. Everytime we made changes to a component in module A we would have to publish the changes and execute `npm update` in our core-project, just to see the changes. And we realy don't want to be publishing things for development purposes. 
...
A slightly less annoying solution might be hosting the module not on the npm registry but on github or a similar platform. Here we would have the possiblity of changing our dependency during development to always take the head of a specified branch for that module. This would enable us to see our changes without having to publish them. It would still mean, that for each change that you make in the module, you would have to commit and push the changes, then execute `npm install` in the core project and rebuild your app. Depending on you typing speed, your internet connection and the efficiency of your build-process, this might take a few minutes. For each tiny change. Obviously, we don't want that either. 
...
npm link
```
This will create a symlink in the global npm directory that links to your module diretory. This means that now, locally on your computer, npm will always know where to find module-a if you wish to link it from any project, anywhere. 
...
If you are managing all of your modules in one single repository it might be worth it to intruduce lerna. Lerna will help you link your packages and publish them with the correct versions. It requires some set up though, so I would advice reading into it a little bit. If you are intersted in Lerna, check it out on [github](https://github.com/lerna/lerna)
